...
path,color,label_name=('./Ch9C2MFNDsample_times.xlsx','dodgerblue','sampling times')
file=pd.read_excel(path)
y= list(file.loc[:,'CM_avg_F1'])
l1=ax1.plot(x,y,c=color,marker='o',label=label_name,markeredgecolor='black',markersize=20)


path,color,label_name=('./Ch9C2MFNDheads_num.xlsx','dodgerblue','crossattention heads')
file=pd.read_excel(path)
y= list(file.loc[:,'CM_avg_F1'])
...
ax1.set_xlabel('(a) sample frequency '+r'$n_{freq}$',fontdict={'family':'Times New Roman','size':48})
...
ax2.set_xlabel('(b) crossattention heads '+r'$n_{head}$',fontdict={'family':'Times New Roman','size':48})
